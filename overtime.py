...
from dataloader import load_raw_data, prepare_overtime_rolling_labeled_loader, prepare_overtime_expanding_labeled_loader
from layers.loss import CustomCrossEntropy
    

def train(args, finetune_date):
    if args.seed:
        torch.manual_seed(args.seed)

    # Set up experiment folder
    exp_name = experiment_name(args)
    year = finetune_date.year
    ...
    if args.use_macro:
        if args.rolling:
            train_data, val_data = prepare_overtime_rolling_labeled_loader(
                train_input, train_target, test_input, test_target, train_mapping_df, test_mapping_df, finetune_date, args.horizon_size, \
                train_company_keys, test_company_keys, company_dict, train_macro_keys, test_macro_keys, macro_dict, **dataloader_kwargs
            )
        else:
            train_data, val_data = prepare_overtime_expanding_labeled_loader(
                train_input, train_target, test_input, test_target, train_mapping_df, test_mapping_df, finetune_date, args.horizon_size, \
                train_company_keys, test_company_keys, company_dict, train_macro_keys, test_macro_keys, macro_dict, **dataloader_kwargs
            )
    else:
    ...
    for i in range(args.epoch):
        ...
        train_loss = 0
        model.train()
        for batch in tqdm.tqdm(train_data, desc=f"{exp_name}: Train Epoch {i}", leave=False, ncols=80):
            optimizer.zero_grad()
            *inputs, y = batch
        ...
        with torch.no_grad():
            val_loss = 0
            for batch in tqdm.tqdm(val_data, desc=f"{exp_name}: Val Epoch {i}", leave=False, ncols=80):
                *inputs, y = batch
                inputs = [input.to(device) for input in inputs]
                y = y.to(device)
                if args.use_macro:
...
def main(args):
    ...
    _ , _= load_raw_data(args.data_dir)

    # Set up the finetune date list
    earliest_date = min(train_mapping_df["date"].min(), test_mapping_df["date"].min())
    finetune_date = earliest_date + pd.DateOffset(months = args.init_overtime_window)
    terminate_date = max(train_mapping_df["date"].max(), test_mapping_df["date"].max())
    print(finetune_date, terminate_date)
    while True:
        train(args, finetune_date)
        finetune_date += pd.DateOffset(months = args.incr_overtime_window)
        if finetune_date > terminate_date:
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--incr_overtime_window", type=int, default=12, help="incremental months of overtime training"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rolling", dest="rolling", action="store_true", help="Enable rolling window (default: enabled)"
    )
    parser.add_argument(
        "--no_rolling", dest="rolling", action="store_false", help="Disable rolling window"
    )
    parser.add_argument(
